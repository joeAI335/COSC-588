...
-- Section #1:
select os, count(*) from agent_logs where os != '' group by os;

-- Section #2: Provide 5 agents for which the OS could not be classified that are bots
select agent from agent_logs where bot AND os = '' AND length(agent) > 15 limit 5;

-- Section #3: Provide 5 agents for which the OS could not be classified that are not bots.
select agent from agent_logs where not bot AND os = '' AND length(agent) > 15 limit 5;
