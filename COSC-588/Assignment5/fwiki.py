...
# pyspark --py-files=fwiki.py
# import fwiki
# char_logs = fwiki.raw_logs(sc).filter(lambda line:"CHAR(45,120,49,50,45,81,45)" in line)
...
# sqlCtx.sql("select distinct ipaddr from logs").collect()
# sqlCtx.sql("select distinct date from logs").collect()
# char_logs = fwiki.raw_logs(sc).filter(lambda line:"CHAR" in line)
# sqlCtx.sql("select distinct request from logs").collect()
